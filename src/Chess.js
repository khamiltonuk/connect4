...
function BoardSquare({ x, y, children }) {
  ...
  const [{ isOver, canDrop }, drop] = useDrop({
    ...
    drop: () => dispatch({ type: "UPDATE_KNIGHT_POSITION", payload: [x, y] }),
    canDrop: () => canMoveKnight(x, y, state.knightPosition),
    collect: (monitor) => ({
...
function renderSquare(i, knightPosition, dispatch) {
  const x = i % 8;
  const y = Math.floor(i / 8);

  function handleSquareClick(toX, toY) {
    if (canMoveKnight(toX, toY, knightPosition)) {
      dispatch({ type: "UPDATE_KNIGHT_POSITION", payload: [toX, toY] });
    }
  }
...
function Board() {
  ...
  const squares = [];
  for (let i = 0; i < 64; i++) {
    squares.push(renderSquare(i, state.knightPosition, dispatch));
  }
